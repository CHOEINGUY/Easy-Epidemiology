// useEpidemicStats.js - 유행데이터 계산 로직 (Refactored)
...
// ─────────────────────────────────────────────────────────────────────────────
const MAX_INTERVALS = 10000; // 안전 제한 (약 1년 분량, 1시간 단위 기준)

function generateOnsetTableData({
  onsetTimes,
  intervalHours,
  firstOnsetTime,
  lastOnsetTime,
  ...
  if (isNaN(blockStartTimestamp)) return [];
  ...
  while (currentIntervalStart <= extendedMaxTimestamp && guard < MAX_INTERVALS) {
...
export function useEpidemicStats() {
  ...
  const exposureDateTime = computed({
    ...
    set: (value) => settingsStore.updateExposureDateTime(value)
  });
  const selectedSymptomInterval = computed({
    get: () => settingsStore.selectedSymptomInterval,
    set: (value) => settingsStore.updateSymptomInterval(value)
  });
  ...
  const floorToIntervalStart = (timestamp, intervalHours) => {
    if (isNaN(timestamp) || isNaN(intervalHours) || intervalHours <= 0) return NaN;
  ...
  // ─────────────────────────────────────────────────────────────────────────
  const isValidPatientRow = (row, requireConfirmed = false) => {
    if (row.isPatient !== '1' || !row.symptomOnset) return false;
    if (requireConfirmed && row.isConfirmedCase !== '1') return false;

    const onset = safeParseDate(row.symptomOnset);
    if (!onset) return false;

    if (isIndividualExposureColumnVisible.value) {
      if (row.individualExposureTime) {
        const exposure = safeParseDate(row.individualExposureTime);
        if (exposure && onset < exposure) return false;
      }
    } else {
      if (exposureDateTime.value) {
        const exposureDate = safeParseDate(exposureDateTime.value);
        if (exposureDate && onset < exposureDate) return false;
      }
    }
    return true;
  };
  ...
  const patientOnsetTimes = computed(() => {
    ...
    return rows.value
      ...
      .map(r => safeParseDate(r.symptomOnset))
      .filter(d => d)
  ...
  // 최초/최종 발생 시간
  const firstOnsetTime = computed(() => patientOnsetTimes.value.length > 0 ? patientOnsetTimes.value[0] : null);
  const lastOnsetTime = computed(() => patientOnsetTimes.value.length > 0 ? patientOnsetTimes.value[patientOnsetTimes.value.length - 1] : null);
