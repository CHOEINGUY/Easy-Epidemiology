...
export async function handleApproveUser(request, env) {
  ...
  try {
    ...
    const updatedUser = {
      ...
      approvedBy: authCheck.user.username
    };
...
// 일괄 사용자 승인
export async function handleBulkApproveUsers(request, env) {
  if (request.method !== 'POST') {
    return errorResponse('Method not allowed', 405);
  }
    ...
    for (const userId of body.userIds) {
      try {
        ...
        try {
          ...
          matches.forEach(match => {
            const [key, value] = match.split(':');
            pendingUser[key.trim()] = value.trim().replace(/^"|"$/g, '');
          });
        }
...
// 일괄 사용자 거부
export async function handleBulkRejectUsers(request, env) {
  if (request.method !== 'POST') {
    return errorResponse('Method not allowed', 405);
  }
...
export async function handleUpdateUserRole(request, env) {
  ...
  // 관리자 권한 확인 (admin만 가능)
  const authCheck = await checkAdminAuth(request, env);
  if (!authCheck.isAdmin || authCheck.user.role !== 'admin') {
  ...
  try {
    ...
    for (const key of allUsers.keys) {
      ...
      try {
        ...
        matches.forEach(match => {
          const [key, value] = match.split(':');
          user[key.trim()] = value.trim().replace(/^"|"$/g, '');
        });
      }
