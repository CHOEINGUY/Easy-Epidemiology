...
export function generateCohortTsv(results) {
  const tableData = [];
...
export function generateCohortHtml(results) {
  const rows = results.map(result => {
    const isSig = result.pValue !== null && result.pValue < 0.05;
    const bgStyle = isSig ? HIGHLIGHT_STYLE : '';
    const pStyle = isSig ? P_VALUE_STYLE : '';
    const pValueText = result.pValue !== null 
...
export function generateCaseControlTsv(results) {
  const tableData = [];
...
export function generateCaseControlHtml(results) {
  const rows = results.map(result => {
    const isSig = result.pValue !== null && result.pValue < 0.05;
    const bgStyle = isSig ? HIGHLIGHT_STYLE : '';
    const pStyle = isSig ? P_VALUE_STYLE : '';
...
export function generateCaseSeriesTsv(results) {
  const tableData = [];
...
export function generateCaseSeriesHtml(results) {
  const rows = results.map(result => `
    <tr>
