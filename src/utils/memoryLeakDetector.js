...
class MemoryLeakDetector {
   ...
   */
  registerComponent(componentInstance, componentName) {
   ...
   */
  registerGlobalReference(componentInstance, referenceName) {
    if (!this.isEnabled) return;
   ...
   * @param {string} referenceName - 참조 이름
   */
  unregisterGlobalReference(componentInstance, referenceName) {
    if (!this.isEnabled) return;
   ...
   */
  startPeriodicCheck(interval = 30000) { // 30초마다
    if (!this.isEnabled) return;
