 ...
 * @returns {boolean}
 */
export function canSafelyCreateWorker() {
  // 기본 워커 지원 확인
 ...
 */
export function createWorkerSafely(workerUrl, options = {}) {
...
export function shouldUseWorkers() {
  // 프로덕션 환경에서만 워커 사용 권장
  return import.meta.env?.MODE === 'production' && canSafelyCreateWorker();
}
 ...
 * @returns {boolean}
 */
export function shouldUseWorker(dataSize = 0) {
  if (!canSafelyCreateWorker()) {
    return false;
  }
