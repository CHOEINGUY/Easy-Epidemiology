// useIncubationStats.js - 잠복기 데이터 계산 로직
...
export function useIncubationStats() {
  ...
  const selectedIncubationInterval = computed({
    ...
    set: (value) => settingsStore.updateIncubationInterval(value)
  });

  // 유틸리티: HH:MM 형식으로 포맷팅
  const formatDurationHHMM = (durationMillis) => {
    if (isNaN(durationMillis) || durationMillis < 0) return '--:--';
