...
export function useGridScrolling(filteredRows, allColumnsMeta, options = {}) {
  ...
  const gridContainerRef = options.gridContainerRef || ref(null);
  const gridHeaderRef = options.gridHeaderRef || ref(null);
  const gridBodyRef = options.gridBodyRef || ref(null);
  ...
  async function ensureCellIsVisible(rowIndex, colIndex) {
    ...
    if (colIndex >= 0) {
      ...
      const colWidth = parseInt(column.style?.width || '0', 10);
