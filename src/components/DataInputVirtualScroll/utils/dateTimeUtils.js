 ...
 */
export function normalizeDateTimeString(value) {
  if (!value || typeof value !== 'string') {
    return '';
  }
 ...
 */
export function isEqualDateTime(dateTime1, dateTime2) {
  let obj1, obj2;
 ...
 */
export function isValidDateTimeRange(startDateTime, endDateTime) {
  let start, end;
  ...
  const startDate = new Date(start.year, start.month - 1, start.day, 
    parseInt(start.hour), parseInt(start.minute));
  const endDate = new Date(end.year, end.month - 1, end.day, 
    parseInt(end.hour), parseInt(end.minute));
 ...
 */
export function tryParseUserInput(input) {
