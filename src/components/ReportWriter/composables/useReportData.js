...
export function useReportData() {
  ...
  const caseAttackRate = computed(() => epidemicStore.getCaseAttackRate);
  const patientAttackRate = computed(() => epidemicStore.getPatientAttackRate);
  ...
  const exposureDate = computed(() => {
    const raw = settingsStore.exposureDate || settingsStore.getExposureDate; // Check store structure, assume getter or state
    if (!raw) return null;
    if (raw.includes('~')) {
  ...
  const meanIncubation = computed(() => {
    const pRows = rows.value.filter(r => r && String(r.isPatient) === '1');
    const isIndividual = settingsStore.isIndividualExposureColumnVisible;
    
    let durations = [];
    
    if (isIndividual) {
      durations = pRows.map(row => {
      ...
      const exposureDateTime = settingsStore.exposureDateTime; // State direct access
  ...
  const hasMainSymptomsTable = computed(() => {
    ...
    const symptomStats = clinicalHeaders.map((symptom, index) => {
      const count = pRows.filter(row => row.clinicalSymptoms && row.clinicalSymptoms[index] === '1').length;
      return count;
  ...
  function generateIncubationExposureText() {
    ...
    const durations = [];
    // exposureDate getter logic:
    // In SettingsStore, check "getExposureDate" or direct "exposureDateTime"
    const exposureSingleStr = settingsStore.exposureDateTime; // Direct state usage for simplicity
  ...
  const firstCaseDateTime = computed(() => {
    const onsets = rows.value.map(r => r?.symptomOnset).filter(Boolean).map(ts => new Date(ts));
